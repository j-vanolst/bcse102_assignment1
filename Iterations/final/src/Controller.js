class Controller {
  static setup () {
    ...
    // }
    // return the2018Games
    let matchInfo = JSON.parse(localStorage.getItem('matchInfo')).sports
    for (let aSport of matchInfo) {
      let newSport = the2018Games.addSport(aSport.name, aSport.venue)
      let matches = aSport.matches
      for (let aMatch of matches) {
        newSport.addMatch(aMatch.year, 3, aMatch.day, aMatch.hour, aMatch.minute,
        aMatch.pool, aMatch.teamA, aMatch.teamB)
      }
    }
    return the2018Games
